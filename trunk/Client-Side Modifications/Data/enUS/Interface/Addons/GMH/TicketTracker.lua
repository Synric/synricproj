...
local function AddTicket(name)
    if not TicketNames[name] then
		...
		if not opentickets then opentickets=opentickets+1;
		else opentickets=opentickets+1; end
    end
end

function TT_Delete(name)
	outSAY( "ticket close "..name );
    Refresh();
    Reticket();
	if not opentickets then opentickets=0;
	else opentickets = opentickets-1; end
end
...
function TT_Click(index)
    TT_Get = true;
    TicketViewPanel1Message:Clear();
	outSAY( "ticket id "..Tickets[(CurrentPage*15)+index].Name );
